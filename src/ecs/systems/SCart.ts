...
export class SCart extends System {
    ...
    private updateFreeCart(
        ...
        this.tryAttach(world, cart, prevPos, pos, vel, rotation, jump)
    }
    ...
    private updateAttachedCart(
        ...
        if (nextDistance <= 0 || nextDistance >= length) {
            ...
            if (!transferred) {
                this.detachAtTrackEnd(track, cart, vel, rotation, tangent, dt, nextDistance, jump)
            }
            return
        }
    ...
    private detachAtTrackEnd(
        ...
        this.detach(cart, vel, releaseVelocity, jump, releaseTangent)
    }
    ...
    update(world: World, _dt: number): void {
        ...
        for (let step = 0; step < time.pendingFixedSteps; step++) {
            ...
            for (const [e, cart, pos, vel] of world.query3(CCart, CPosition, CVelocity)) {
                ...
                if (!cart.attached) {
                    this.updateFreeCart(world, e, cart, pos, vel, dt, rotation)
                    continue
                }

                this.updateAttachedCart(world, e, cart, pos, vel, dt, rotation)
            }
        }
    }

    private detach(cart: CCart, vel: CVelocity, releaseVelocity: Vector3, jump?: CJump, coyoteTangent?: Vector3) {
    ...
    private tryAttach(
        ...
        if (jump) {
            ...
            if (SJump.tryConsumeQueuedTrackJump(cart, jump, vel, attachSample.tangent)) {
                return
            }
        }
    }
